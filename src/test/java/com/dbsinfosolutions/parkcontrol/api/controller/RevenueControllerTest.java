...
class RevenueControllerTest {
    ...
    void returnsRevenueForProvidedDate() throws Exception {
        ...
        mockMvc.perform(get("/revenue").param("date", "2026-07-04").param("sector", "A"))
                .andExpect(status().isOk()).andExpect(jsonPath("$.amount").value(12.50))
    ...
    void defaultsDateToTodayWhenMissing() throws Exception {
        ...
        when(getRevenueUseCase.execute(eq(today), eq("A"))).thenReturn(new RevenueResponse(
                BigDecimal.ZERO, "BRL", Instant.parse("2026-07-04T12:00:00Z"), "A"));
